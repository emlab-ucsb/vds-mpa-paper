...
effort_by_vessel %>% 
  ...
  mutate(date = lubridate::date(paste(year, month, "01", sep = "/")),
         post = year >= 2015) %>% 
  select(-year) %>% 
  gather(variable, value, -c(date, month, post, gear)) %>% 
  ggplot(aes(x = date, y = value)) +
  geom_line() +
  geom_point(aes(color = post)) +
  facet_wrap(gear~variable, scales = "free_y", ncol = 3) +
  theme_bw() +
  scale_color_brewer(palette = "Set1")
```
...
kir_vessel <- effort_by_vessel %>% 
  filter(kir, gear == "purse_seines") %>% 
  mutate(post = post * 1,
         treated = treated * 1)

m1 <- lm(hours ~ post*treated, data = kir_vessel)
m2 <- lm(hours ~ post*treated + month, data = kir_vessel)
m3 <- lm(hours ~ post*treated + month + year, data = kir_vessel)
m4 <- lm(hours ~ post*treated + month + year + mmsi, data = kir_vessel)
...
m1 <- lm(hours ~ post*treated, data = effort_by_vessel)
m2 <- lm(hours ~ post*treated + month, data = effort_by_vessel)
m3 <- lm(hours ~ post*treated + month + year, data = effort_by_vessel)
m4 <- lm(hours ~ post*treated + month + year + mmsi, data = effort_by_vessel)
...
map_df(models, broom::tidy, .id = "model") %>%
  filter(term %in% c("(Intercept)", "postTRUE", "treatedTRUE", "postTRUE:treatedTRUE")) %>%
