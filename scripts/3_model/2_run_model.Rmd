...
```{r}
no_trade_bm <- DF_results_no_trading %>% 
  filter(R == 0, theta == 0) %>%
  select(
    Country,
    b_harvest = Harvest,
...
DF_1_no_trading <- DF_results_no_trading %>%
  filter(Country == "KIR") %>%
  left_join(no_trade_bm, by = "Country") %>%
  mutate(
...
ggsave(plot = costs_no_trading_plot,
       filename = here("docs", "diss_img", "cost_no_trading.png"),
       width = 6,
       height = 4.5)
```
...
```{r}
trading_bm <- DF_results_with_trading %>% 
  filter(R == 0, theta == 0) %>%
  select(
    Country,
    b_harvest = Harvest,
...
DF_1_with_trading <- DF_results_with_trading %>%
  filter(Country == "KIR") %>%
  left_join(trading_bm, by = "Country") %>%
  mutate(
...
ggsave(plot = costs_with_trading_plot_long_axis,
       filename = here("docs", "diss_img", "cost_with_trading_long_axis.png"),
       width = 6,
       height = 4.5)
```
...
ggsave(plot = costs_with_trading_plot,
       filename = here("docs", "diss_img", "cost_with_trading.png"),
       width = 6,
       height = 4.5)
```
...
ggsave(plot = delta_costs_plot,
       filename = here("docs", "diss_img", "panel2c.pdf"),
       width = 3.4, height = 2.3)

ggsave(plot = delta_costs_plot,
       filename = here("docs", "diss_img", "panel2c.png"),
       width = 6,
       height = 4.5)
```
...
ggsave(plot = cost_plot,
       filename = here("docs", "diss_img", "PNA_model.pdf"),
       width = 3.4,
       height = 7)
```
...
ggsave(plot = effort_and_revenues,
       file = here("docs", "diss_img", "effort_and_revenues.pdf"),
       width = 6,
       height = 4)
```
...
```{r}
ggsave(plot = effort_and_revenues,
       here("docs", "diss_img", "effort_and_revenues_ED_Fig4.tiff"),
       width = 6,
       height = 4)
```
...
ggsave(plot = vessel_day_price_no_trading_plot,
       filename = here("docs", "diss_img", "vessel_day_price_no_trading_plot.pdf"),
       width = 6,
       height = 3)
```
...
```{r}
ggsave(plot = vessel_day_price_no_trading_plot,
       here("docs", "diss_img", "vessel_day_price_no_trading_plot_ED_Fig1.tiff"),
       width = 6,
       height = 3)
```
...
ggsave(plot = vessel_day_price_with_trading_plot,
       here("docs", "diss_img", "vessel_day_price_with_trading_plot.pdf"),
       width = 6,
       height = 4.5)
```
...
```{r}
ggsave(plot = vessel_day_price_with_trading_plot,
       here("docs", "diss_img", "vessel_day_price_with_trading_plot_ED_Fig3.tiff"),
       width = 6,
       height = 4.5)
```
...
ggsave(plot = profits_PNA_notKIR_no_trading_plot,
       here("docs", "diss_img", "profits_PNA_notKIR_no_trading_plot.pdf"),
       width = 3,
       height = 3)
```
...
```{r}
ggsave(plot = profits_PNA_notKIR_no_trading_plot,
       here("docs", "diss_img", "profits_PNA_notKIR_no_trading_plot_ED_Fig2.tiff"),
       width = 3,
       height = 3)
```
...
ggsave(
  ...
  here("docs", "diss_img", "allocation_cost_plot.pdf"),
  width = 5,
  height = 3.5
)
...
ggsave(
  plot = allocation_cost_plot,
  here("docs", "diss_img", "allocation_cost_plot.png"),
  width = 6,
  height = 3.5
)
```
...
```{r}
# revenues increase between 0\% and 7\% each
DF_1_no_trading %>% pull(rel_cost_pna_not_kir) %>% range() * 100

# When trading between countries occurs, 88.5\% to 99.2\% of revenue losses can be avoided.
both %>% pull(rel_delta_cost) %>% range() * 100

# implementing a reserve results in long-term losses to the conserving country of 20\% to 93\%, depending on the size of reserve and stock movement
allocation %>%
    filter(R > 0,
           trade) %>% 
    filter(alpha == 1) %>% 
...
#However, a biomass-only allocation rule (\emph{i.e.} $\alpha = 0$) results in revenue losses of up to 0.7\%, essentially eliminating the costs of conservation.
allocation %>%
    filter(R > 0,
           trade) %>% 
    filter(alpha == 0) %>% 
