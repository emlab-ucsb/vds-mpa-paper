###############################################
#   analyses_on_controls_random_treatment    #
##############################################

#########################################################
# This fits all models for data where the treatment is
# randomly assigned (with a 50/50 probability)
#########################################################
...
effort_by_vessel <- readRDS(file = here::here("data",
  ...
  mutate(treated = rbinom(n = nrow(.), size = 1, prob = 0.5) > 0)
...
# Export figure
ggsave(plot, filename = here::here("docs", "img", "other_specifications_random_treatment.pdf"), width = 6.1, height = 8)
