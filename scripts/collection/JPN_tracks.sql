-- This gets tracks for all Japanese purse seiners. Vessels that continously fished must still be identified, but I can do that in R
SELECT
  A.mmsi AS mmsi,
...
FROM (
  ...
  AND
  mmsi IN (SELECT mmsi FROM [world-fishing-827.gfw_research.vessel_info] WHERE iso3 = "JPN" AND best_label = "purse_seines") --vessels that are from Taiwan and are purse seiners
  AND
  ...
  AND
  lon NOT BETWEEN -67 AND 128 AND lat BETWEEN 0 AND 60 --Filter for data in the Pacific only
  ) A
JOIN (
  SELECT
    mmsi,
