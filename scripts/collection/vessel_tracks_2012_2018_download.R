#########################################
#   vessel_tracks_2012_2018_download    #
#########################################

########################################################################
# This script downloads the vessel_tracks_2012_2018 dataset from Google
# BigQuery. I queried the database in GBQ to get the tracks of all
# vessels we need using the updated pipe_production_b_fishing.
# 
...
vessel_tracks <- dplyr::tbl(BQc, "vessel_tracks_2012_2018") %>%
...
vessel_tracks <- vessel_tracks %>% 
  ...
  select(
    ...
    fishing,
    eez_iso3,
    distance_from_port = distance_from_port_m,
    distance_from_shore = distance_from_shore_m,
    month_c,
    year_c,
    year_month,
    quarter,
...
# Save it to disk
saveRDS(vessel_tracks, file = here::here("raw_data", "vessel_tracks_2012_2018.rds"))
