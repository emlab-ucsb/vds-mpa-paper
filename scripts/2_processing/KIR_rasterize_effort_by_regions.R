#######################################
#   KIR_rasterize_effort_by_regions   #
#######################################
...
# which I export as tiff. It does it only for KIRIBATI
######################################################################
...
monthly_effort_raster <- data.frame(x = NULL,
                                    ...
                                    month = NULL)
for(j in groups){
  for(i in 17:length(dates)){
    # Get the points for month i in group j
    data_j_i <- monthly_vessel_tracks %>% 
      filter(date == dates[i],
             treated == j)
    
    if(nrow(data_j_i) == 0){raster_j_i[,] <- 0}else{
...
saveRDS(monthly_effort_raster,
        file = here::here("data", "KIR_monthly_rasterized_effort_by_region.rds"))
